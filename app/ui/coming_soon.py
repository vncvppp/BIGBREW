...
ASSETS_PATH = PROJECT_ROOT / "resources" / "coming_soon"
...
def _open_script(script_name: str):
    try:
        module_name = Path(script_name).with_suffix("").name
        subprocess.Popen([sys.executable, "-m", f"app.ui.{module_name}"], cwd=str(PROJECT_ROOT))
    finally:
        try:
            window.destroy()
        except Exception:
